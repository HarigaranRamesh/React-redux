...
class UserService extends crud_service {
  ...
  async updateUser(userId, data) {
    try {
      ...
      delete data._id;
      delete data.password; // Use change-password API for this

      // 🟢 FIX: If role is a name (e.g. "Admin"), look up the ID
      if (data.role && typeof data.role === 'string' && !data.role.match(/^[0-9a-fA-F]{24}$/)) {
        const roleDoc = await this.access_model.findOne({ role: data.role });
        if (roleDoc) {
          data.role = roleDoc._id;
        } else {
          console.warn(`Role '${data.role}' not found. Removing from update payload.`);
          delete data.role; // or throw error
        }
      } else if (data.role && typeof data.role === 'object' && data.role._id) {
        // If it's an object (populated), use the _id
